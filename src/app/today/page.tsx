...
export default function TodayPage() {
    ...
    const [isCustomStream, setIsCustomStream] =
        useState(false);

    // New: track classes added during the current modal session
    const [newlyAddedClasses, setNewlyAddedClasses] =
        useState<any[]>([]);

    const [classSaveMessage, setClassSaveMessage] =
        useState<string | null>(null);

    const classNameInputRef =
        useRef<HTMLInputElement>(null);

    // ---------------------------------------------------------
    // SUSPENSION MODAL
    // ---------------------------------------------------------
    const [isSuspendModalOpen, setIsSuspendModalOpen] =
        useState(false);
    ...
    const [topicCovered, setTopicCovered] =
        useState('');
    ...
    useEffect(() => {
        ...
        try {
            ...
            console.error(
                'Failed to load ProfPlan local store data:',
                err
            );
    ...
    // ---------------------------------------------------------
    // TODAY'S DATE
    // IMPORTANT: standard YYYY-MM-DD format
    // ---------------------------------------------------------
    ...
    // Starts a fresh multi-class entry session
    // ---------------------------------------------------------
    ...
    // Modal intentionally remains open after saving
    // ---------------------------------------------------------
    const handleSaveNewClass = (
        ...
        if (!className || !streamName) {
            setClassSaveMessage(
                'Please enter the Class / Semester and select or enter a Stream / Faculty / Branch.'
            );
            return;
        }

        // Prevent duplicate Class/Semester + Stream entries
        ...
        if (duplicateExists) {
            setClassSaveMessage(
                'This Class / Semester with the same Stream / Faculty / Branch is already registered.'
            );
        ...
        try {
            ...
            setClassSaveMessage(
                `"${className}" has been added successfully. You can now add the next class / semester.`
            );

            // Keep modal open and focus next entry
            setTimeout(() => {
                classNameInputRef.current?.focus();
            }, 100);
            ...
            console.error(
                'Error saving new class:',
                err
            );

            setErrorMessage(
                'Failed to save class to storage. Please try again.'
            );
        }
    };
    ...
    const handleDeleteClass = (
        ...
        try {
            ...
            // Also remove from current session list
            setNewlyAddedClasses(
                prev =>
                    prev.filter(
                        c =>
    ...
    const handleRecordNonInstructionalDay =
        (
            ...
            try {
                ...
                console.error(
                    'Failed to record suspension/holiday status:',
                    err
                );

                setErrorMessage(
                    'Failed to save suspension record. Storage quota or format error.'
                );
            }
        };
    ...
    const handleRemoveSuspensionLog =
        () => {
            ...
            try {
                ...
                console.error(
                    'Failed to remove suspension status:',
                    err
                );
    ...
    const handleSaveQuickLog = (
        ...
        try {
            ...
            const newLog: any = {
                ...
                plannedTopicName:
                    plannedTopicObj?.name || plannedTopicObj?.title ||
                    'General / Unplanned',
            ...
            setErrorMessage(
                'Failed to save progress log. Please check your inputs or storage quota.'
            );
        }
    };
                            ...
                            >
                                ...
                                <div>
                                    ...
                                    <p className="mt-1.5 text-[11px] text-slate-500 font-medium">
                                        Choose a stream/faculty/branch or add your own.
                                    </p>
                                </div>
                            ...
                            (
                                ...
                                return (
                                    ...
                                    <div
                                        ...
                                        className={`rounded-3xl border p-5 shadow-sm transition flex flex-col md:flex-row md:items-center md:justify-between gap-4 ${isDone
                                    ...
                                    >
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <div className="flex flex-wrap items-center gap-2">
                                                ...
                                                <span
                                                    className={`px-2.5 py-0.5 rounded-md font-black text-xs ${isDone
